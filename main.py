...
def main():
    ...
    GAP_WORSE = [400, 600, 800, 1000]    
    PERC_UNROUTED = [0.1, 0.2, 0.3]
    TABU_LENGTH = [50, 70, 90, 110, 130, 150]

    for gap_worse in GAP_WORSE:
        for perc_unrouted in PERC_UNROUTED:
    ...
    # ------------------------------------------------ INITIALIZATION -------------------------------------------------------
            ...
            # ------------------------------------------------- SIMULATION ---------------------------------------------------------
                ...
                for day in range(n_days):
                    ...
                    while not(solution):
                        ...
                        if solver == 'ortools':
                            ...
                            if solution:
                                tabu_search = TabuSearch(clark_wright_sol, constant.MAX_TIME, tabu_len, gap_worse, perc_unrouted)                            
                                ii=0
                ...
                print('Gap Worse {} Tabu Length {} Perc Unrouted {}'.format(gap_worse, tabu_len, perc_unrouted))
